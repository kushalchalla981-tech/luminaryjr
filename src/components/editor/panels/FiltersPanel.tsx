    ...
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-2 gap-4 overflow-y-auto pb-10 custom-scrollbar pr-2">
        {filterCategories[0].filters.map((filter) => (
          ...
          >
            <div className="w-full aspect-square rounded-xl bg-slate-800 border border-slate-200 dark:border-white/10 overflow-hidden relative transition-all group-hover:border-yellow-500/50 group-hover:shadow-[0_0_20px_rgba(255,215,0,0.3)]">
            ...
            <span className="text-xs font-medium text-slate-500 dark:text-slate-500 dark:text-slate-400 group-hover:text-slate-900 dark:text-slate-900 dark:text-white transition-colors text-center">
              {filter.name}
            </span>
          </div>
        ))}
