        ...
        return (
          <div key={ctrl.id} className="group flex flex-col gap-2">
            <div className="flex justify-between items-end">
              <label 
                htmlFor={ctrl.id} 
                className="text-xs font-semibold tracking-wide text-slate-400 group-hover:text-white transition-colors"
              >
                {ctrl.label}
              </label>
            ...
            <div className="relative w-full h-1.5 flex items-center">
              ...
              {ctrl.min < 0 && (
                <div className="absolute left-1/2 top-1/2 h-2.5 w-0.5 -translate-x-1/2 -translate-y-1/2 bg-white/20 z-0 rounded-full" />
              )}
